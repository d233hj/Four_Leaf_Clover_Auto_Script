...
class AdbTool:
    ...
    def research_img(self, template):
        ...
        if self.__center != False:
            self.setMsg("#-found img:" + str(self.__center) + "\n -" + str(self.__max_val))
            return True
        else:
            self.setMsg("#-img not found:" + str(self.__center) + "\n -" + str(self.__max_val))
            return False

    def apper_to_click(self, template):
        "匹配图片，如果存在则点击中心\ntemplate:需要查找的图片"
        self.setMsg("None")
        self.__take_screenshot()
        screen = cv2.imread(self.__img_where)
        ...
        if self.__center != False:
            ...
            self.setMsg("#~found img:<" + argname('template') + ">, " + str(self.__center) + "\t " + str(self.__max_val))
            print(self.getMsg())
            return True
        else:
            self.setMsg("#-img not found:" + str(self.__center) + "\n -" + str(self.__max_val))
