...
def World_Tree(chufamaoxian,loading,kaishibiaoyan,qiyu,xiuxi,tuzishangdian,huode,baoxiang,zhanbu,zhandou,shezhi,fangqizhandou,fangqi,xuanze
               ,sanxuan,jiesugoumai,sure,nSure):
    ...
    try:
        while (1):
            ...
            print("进入战斗了：")
            pos_zhandou,val_zhandou=image_to_position(check,zhandou)
            print("奇遇：")
            ...
            print("*"*100)
            if(pos_zhandou and val_zhandou>0.8):
                print("开始战斗中")
                pos_zhandou,_=image_to_position(check,zhandou)
                adb_click(pos_zhandou)
                while(1): 
                    take_screenshot()
                    pos_nSure,val_nSure=image_to_position(cv2.imread("tmp.png"),nSure)
                    if(pos_nSure and val_nSure >0.8):
                        adb_click((1300,400))
                        time.sleep(1)
                        adb_click(pos_nSure)
                        break
...
if __name__ == '__main__':
    ...
    zhandou =cv2.imread("zhandou.png")
    ...
    sure= cv2.imread("sure.png")
    nSure= cv2.imread("nSure.png")
    for i in range(150):
        print("-"*100)
        print("第{}次世界树副本探索中".format(i+1))
        print("-"*100)
        World_Tree(chufamaoxian,loading,kaishibiaoyan,qiyu,xiuxi,tuzishangdian,huode
                   ,baoxiang,zhanbu,zhandou,shezhi,fangqizhandou,fangqi,xuanze
                    ,sanxuan,jiesugoumai,sure,nSure)
        i+=1
        print("-"*45+"已完成！！！"+"-"*48)
        time.sleep(4)
